...
As seems to usually be the case when comparing mixer and CDJ packets sent to port 50000, the difference between this and <<#mixer-keep-alive,the mixer version>> is that byte{nbsp}``25``
(following the device number `D`) has the value `01` rather than `02`, and the same is true of the second-to-last byte in each of the packets.
(Byte{nbsp}``34`` is `01` here and and `02` in <<#mixer-keep-alive,the mixer version>>.)
