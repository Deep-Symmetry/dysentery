...
_len~r~_ specifies the length of data following the length field,
currently seen as `06`.
...
to a timing packet in which _E_ is set to `01`. `002c` packets are sent
in response to a timing packet in which _E_ is set to `00`.
