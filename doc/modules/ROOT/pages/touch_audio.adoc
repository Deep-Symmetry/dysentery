...
(DJM900NXS2 starts emitting timing packets roughly after 19 seconds from
...
Audio Data packets are sent from players to the mixer.

NOTE: Irrespective of the _len~r~_ field, this packet is always 1388
bytes long.
...
_len~r~_ specifies the length of the packet, either `056c` (header plus
audio data) or `002c` (header only). `056c` packets are sent in response
to a timing packet in which _E_ is set to `01`. `002c` packets are sent in
response to a timing packet in which _E_ is set to `00`.
...
_C_ value of counter received from the mixer that the player is
responding to.

_PCM Data_ 1344 bytes (336 samples) of uncompressed PCM, signed 16-bit,
