...
database present). The purpose of value _x_ at byte{nbsp}``a8`` is
uncertain, we have seen values of `06` and `00`, but don’t know what
they mean.
...
associated with this media. Again, we don’t know the purpose of _a_ at
byte{nbsp}``ab``, though it seems that it might have the value `01`
when there is a rekordbox database present, and `00` otherwise, hence
the use of _a_ to suggest “analyzed.”
