...
Zulip stream] or--if you are willing to fork the project and update
this document, a pull request--would be extremely welcome). Relatedly,
when a track is loaded from a disc, playlist, or menu, the field
_d~n~_ at bytes{nbsp}``46``–`47` changes from `0000` to the number of
tracks on the disc or in the playlist or menu (a data disc will
generally have one track).
...
us know on
https://deep-symmetry.zulipchat.com/join/vek5i5d5f23j6ahbyomv3waf/[Zulip]
