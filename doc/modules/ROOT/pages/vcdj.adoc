...
(draw-box (text "Key") {:span 3})
(draw-box 1)
(draw-padding 0x164 nil)
(draw-box (text "KeyShift") {:span 8})
...
Bytes `11d` and `11e` (labeled _Buf~f~_ and  _Buf~b~_ respectively) seem
...
Byte `11f` (labeled _Buf~s~_) changes to value `01` when the entire track
is buffered in memory.

Bytes `15c` to `15e` (labeled _Key_) indicate the key of the loaded track.
Byte `15c` represents the note, byte `15d` is `00` for minor and `01` for
major, and byte `15e` is `00` for natural, `01` for sharp, and `ff` for
flat.
...
Bytes `164` to `16b` (labeled _KeyShift_) is a 64-bit signed value
indicating the Key Shift value of a CDJ-3000. The value changes in
increments of `64` for each semitone moved.
