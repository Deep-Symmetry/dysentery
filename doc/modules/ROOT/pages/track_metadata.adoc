...
following four messages. (You can also get metadata for non-rekordbox
...
track, is the little-endian integer at bytes{nbsp}``25``–`28`. The
_B~b~_ value for the third beat is at byte{nbsp}``34``, and its
millisecond time is at bytes{nbsp}``35``–`38`, and so on.
