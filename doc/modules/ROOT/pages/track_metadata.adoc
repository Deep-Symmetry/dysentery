...
to correspond to “half frames” counted as `03.5~F~` following the seconds
in the player display.) Each byte encodes both a color and height. The
three high-order bits encode the color, ranging from darkest blue at 0
to near-white at 7. The five low-order bits encode the height of the
...
are the slot in which the track being asked about can be found and its
track type; each has the same values used in
<<vcdj#cdj-status-packet,CDJ status packets>>. For some reason the
...
be found in the `segmentColor` and `segmentHeight` methods of the
...
representation can be found in the `updateWaveform` method of the
...
to correspond to “half frames” counted as `03.5~F_` following the
seconds in the player display.) Each pair of bytes encodes the height
of the waveform at that segment as a five bit value, along with three
bits each of red, green, and blue intensity, arranged as shown below:
...
are the slot in which the track being asked about can be found and its
track type; each has the same values used in
<<vcdj#cdj-status-packet,CDJ status packets>>. The value after _D_,
...
(draw-box (text "cue" :math) {:span 4})
(draw-box (text "loop" :math) {:span 4})
(draw-related-boxes (repeat 0x34 nil))
...
(draw-related-boxes (repeat 0x14 nil))
----
...
The actual location of the cue and loop points are in the values _cue_
and _loop_. These are both 4-byte integers, and as noted above, they
...
Immediately after _comment_ (in other words, starting _len~c~_ + `4a`
past the start of the entry) there are four one-byte values containing
color information. 𝑐 appears to be a code identifying the color with
which rekordbox displays the cue, by looking it up in a table. There
have been sixteen codes identified, and their corresponding RGB colors
can be found by looking at the `findRecordboxColor` static method in
the Beat Link library’s
...
two bytes have the same values used in <<vcdj#cdj-status-packet,CDJ
...
CDJs, explored in more breadth in TODO: Section 7. The player responds
with a success indicator, saying it is ready to send these “menu
items” and reporting how many of them are available, much like shown
in the <<metadata-available-response,metadata available response>>,
although the first argument will be `1004` to reflect the message type
we just sent, rather than `2002` as it was for the metadata request.
...
asked about can be found, and _T~r~_ is the type of the track; these
two bytes have the same values used in <<vcdj#cdj-status-packet,CDJ
status packets>>. You specify the ID of the playlist or folder you
want to list in the _id_ argument, and set _folder?_ 1 if you are
asking for a folder, and 0 if you are asking for a playlist. As noted
above, to get the top-level list of playlists, ask for folder 0, by
passing an _id_ of 0 and passing _folder?_ as 1.
