...
send 208-byte packets with slightly less information.
...
When a rekordbox track is being played, the device holding the
rekordbox database from which the track was loaded is reported in
$D_r$ (if the track was loaded from the local device, this will be the
same as $D$; if it was loaded over the Link, it will be the number of
a different device). Similarly, $S_r$ reports the slot from which the
track was loaded: The value {\tt 01} means the CD drive, {\tt 02}
means the SD slot, and {\tt 03} means the USB slot. $t_r$ may be an
indicator of whether the track came from rekordbox: It seems to have
the value {\tt 01} when a rekordbox track is loaded, and {\tt 05} when
a regular audio CD is loaded.

The field $rekordbox$ contains the rekordbox database ID of the loaded
track when a rekordbox track is being played. Combined with the player
number and slot information, this can be used to request the track
metadata as described in Section~\ref{sec:metadata}. When an audio CD
is loaded, this just contains the track index on the disc.
...
206, connecting to port 12523, looks like it may be connecting to the
rekordbox database on the USB in the first CDJ, but since the Link
button was not pressed during the capture, that session was not used
beyond its initial setup. That is another potential area for study
someday.
...
three other tracks. It looks like the track names are probably sent in
a UTF-16 encoding? And presumably nearby there is is information about
the key, BPM, star rating, and so on.
...
Although we don't know all the details yet, this is enough to start
adding metadata support to dysentery and beat-link; please dive in and
help us figure out more details, how to look up metadata for tracks
loaded from rekordbox on a laptop, and so on!
...
Now that we've discovered how to get track metadata, it looks like a
very similar query can be used to obtain the album art; the second
capture discussed in Section~\ref{sec:metadata} has some examples.

Before we discovered the way to ask players for metadata about
