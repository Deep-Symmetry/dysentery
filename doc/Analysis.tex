...
There are two places that might contain pitch values, bytes~40--43 and
bytes~48--51, but since they always {\tt 0x100000} (or $+0\%$), we
can't be sure. The first value is structurally in the same place with
respect to $BPM$ as it is found in all other packets containing pitch
information, so that is the one we are assuming is definitive. In any
case, since it is always $+0\%$, the current tempo in beats-per-minute
identified by the mixer can be obtained as:
...
\begin{longtabu}{rX}
  ...
  0 & No track is loaded \label{table:p1Values} \\
...
about the current play mode. The meanings that we have
found so far are listed in Table~\ref{table:p3Values}.
  ...
  \begin{longtabu}{rX}
    ...
    0 & No track is loaded \label{table:p3Values} \\
...
between 0x13 and 0x1b identifying the color, if any, assigned to the
track (see Section~\ref{sec:menuItemTypes} for the color choices), and
if the value is anything other than {\tt 0x13}, Argument 4 contains
the text that the DJ has assigned for that color meaning in rekordbox.
...
\begin{longtabu}{rl}
  ...
  {\tt 0x0001} & \label{table:menuItemTypes}Folder (such as in the
  ...
  {\tt 0x0016} & Color Orange \\

  {\tt 0x0017} & Color Yellow \\

  {\tt 0x0018} & Color Green \\

  {\tt 0x0019} & Color Aqua \\

  {\tt 0x001a} & Color Blue \\

  {\tt 0x001b} & Color Purple \\

  {\tt 0x0023} & Comment \\

  {\tt 0x0028} & Original Artist \\

  {\tt 0x0029} & Remixer \\

  {\tt 0x002e} & Date Added \\

  {\tt 0x0204} & Track title and album \\

  {\tt 0x0604} & Track Title and Genre \\

  {\tt 0x0704} & Track Title and Artist \\

  {\tt 0x0a04} & Track Title and Rating \\

  {\tt 0x0b04} & Track Title and Time \\

  {\tt 0x0d04} & Track Title and BPM \\

  {\tt 0x0e04} & Track Title and Label \\

  {\tt 0x0f04} & Track Title and Key \\

  {\tt 0x1004} & Track Title and Bit Rate \\

  {\tt 0x1a04} & Track Title and Color \\

  {\tt 0x2304} & Track Title and Comment \\

  {\tt 0x2804} & Track Title and Original Artist \\

  {\tt 0x2904} & Track Title and Remixer \\

  {\tt 0x2a04} & Track Title and DJ Play Count \\

  {\tt 0x2e04} & Track Title and Date Added \\
...
\begin{longtabu}{rcX}
  ...
  7 & number & type of this item, {\tt 0x704} for Title and Artist \\
...
\begin{longtabu}{rcX}
  ...
  1 & {\tt 0x704} & Title and Artist sorted by
  title \label{table:sortOrders} \\

  2 & {\tt 0x704} & Title and Artist sorted by artist \\

  3 & {\tt 0x204} & Sorted by album, Arg 1 holds album ID, Label~2
  holds album name \\

  4 & {\tt 0xd04} & Sorted by BPM, Arg 1 holds BPM$\times$100, Label~2
  empty \\

  5 & {\tt 0xa04} & Sorted by rating, Arg 1 holds rating, Label~2
  empty \\

  6 & {\tt 0x604} & Sorted by genre, Arg 1 holds genre ID, Label~2
  holds genre name \\

  7 & {\tt 0x2304} & Sorted by comment, Arg 1 holds comment ID,
  Label~2 holds comment \\

  8 & {\tt 0xb04} & Sorted by time, Arg 1 holds track length in
  seconds, Label~2 empty \\

  9 & {\tt 0x2904} & Sorted by remixer, Arg 1 holds remixer ID,
  Label~2 holds remixer \\

  10 & {\tt 0xe04} & Sorted by label, Arg 1 holds label ID, Label~2
  holds label \\

  11 & {\tt 0x2804} & Sorted by original artist, Arg 1 holds original
  artist ID, Label~2 holds original artist \\

  12 & {\tt 0xf04} & Sorted by key, Arg 1 holds key ID, Label~2 holds
  key text \\

  13 & {\tt 0x1004} & Sorted by bit rate, Arg 1 holds bit rate,
  Label~2 empty \\

  16 & {\tt 0x2a04} & Sorted by DJ play count, Arg 1 holds play count,
  Label~2 empty \\

  17 & {\tt 0x2e04} & Sorted by date added, Arg 1 holds date ID,
