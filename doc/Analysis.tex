...
\ifxetex
    ...
    %\setmonofont{Andale Mono}  Has no bold version, which we rely on.
    %\setmathfont
    %    [ Extension = .otf,
    %      BoldFont = *bold,
    %    ]{xits-math}
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.5em,boxformatting={\baselinealign}]{16}
    ...
    \end{rightwordgroup} \\
    \bitbox{1}{\tt 01} & \bitbox{1}{\tt \textbf{02}} & \bitbox{2}{$len_p$} & \bitbox{1}{\tt 02}
  \end{bytefield}
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.5em,boxformatting={\baselinealign}]{16}
    ...
    \begin{rightwordgroup}{Name}
      ...
      \wordbox[lrb]{1}{Device Name (padded with {\tt 00})}
    \end{rightwordgroup} \\
    \bitbox{1}{\tt 01} & \bitbox{1}{\tt \textbf{02}} & \bitbox{2}{$len_p$} &
    \bitboxes*{1}{{$N$} {\tt 02}} & \bitbox{6}{MAC address}
  \end{bytefield}
...
\begin{figure}[ht]
  \begin{bytefield}[bitwidth=1.5em,boxformatting={\baselinealign}]{16}
    ...
    \begin{rightwordgroup}{Name}
      ...
      \wordbox[lrb]{1}{Device Name (padded with {\tt 00})}
    \end{rightwordgroup} \\
    \bitbox{1}{\tt 01} & \bitbox{1}{\tt \textbf{02}} & \bitbox{2}{$len_p$} & \bitbox{1}{\tt 01}
  \end{bytefield}
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
...
reported. \emph{Note that the timing values for all these upcoming
  beats are always reported as if the track was being played at normal
  speed, with a pitch adjustment of 0\%. If a pitch adjustment is in
  effect, you will need to perform the calculation to scale the beat
  timing values yourself.} $nextBeat$ at bytes~{\tt 24}--{\tt 27} is
the number of milliseconds in which the very next beat will arrive.
$2ndBeat$ (bytes~{\tt 28}--{\tt 2b}) is the number of milliseconds
until the beat after that. $nextBar$ (bytes~{\tt 2c}--{\tt 2f})
reports the number of milliseconds until the next measure of music
begins, which may be from 1 to 4 beats away. $4thBeat$ (bytes~{\tt
  30}--{\tt 33}) reports how many millisecond will elapse until the
fourth upcoming beat; $2ndBar$ (bytes~{\tt 34}--{\tt 37}) the interval
...
have reached); and $8thBeat$ (bytes~{\tt 38}--{\tt 3b}) tells how many
millieconds we have to wait until the eighth upcoming beat will
arrive.
...
\notebox{But note that use of a non-standard player number (outside
  the range 1--4) will interfere with your ability to perfrom metadata
  requests using {\tt dbserver} queries as described in
  Section~\ref{sec:dbServerMessages}. In situations where there are
  four actual players on the network you can use alternate ways to get
  the data, as described in Section~\ref{sec:dealingWithFourPlayers}.}
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{03}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
    ...
    \begin{leftwordgroup}{\tiny\bfseries 30}
      \bitboxes*{1}{{\tt 00} {\tt 00}} \bitbox{2}{$Track$} \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00} {$d_l$}
        {\tt 00} {\tt 00} {\tt a0} {\tt 00} {\tt 00} {\tt 00} {\tt 00} {\tt 00}}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 40}
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00} {\tt 00} {\tt 00} {\tt 00}} & \bitbox{2}{$d_n$} &
...
The field~$d_l$ at byte~{\tt 37} was given this label because we first
believed it to indicate when a disc is loaded. It has the value~{\tt
  00} when the disc slot is empty, and seems to have the value~{\tt
  1e} when a CD Audio disc is loaded, and {\tt 11} when a data disc
containing files in MP3, AAC, WAV or AIFF format is loaded. However,
we later noticed that it also gets non-zero values when a track is
loaded from a playlist ({\tt 05}) or another player menu (different
values, depending on the type of menu; if anyone would like to take
the time to record all the different values and their meanings, a pull
request would be extremely welcome). Relatedly, when a track is loaded
from a disc, playlist, or menu, the field~$d_n$ at bytes~{\tt
  46}--{\tt 47} changes from {\tt 0000} to the number of tracks on the
disc or in the playlist or menu (a data disc will generally have one
track).
...
{\tt fe}, and the XDJ-XZ uses the values {\tt 9a} and {\tt 9e} so this
seems to be another bit field like $F$.
...
nexus players, {\tt 1f} for the XDJ-XZ, and {\tt 05} for older
players.
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$D$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$S$} & \bitbox[]{4}{}
    \end{leftwordgroup}
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$D$} & \bitbox[]{8}{}
    \end{leftwordgroup}
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$D$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00} {\tt 01}} & \bitbox[]{4}{}
    \end{leftwordgroup}
...
``magic`` value (it is always {\tt 872349ae}). This is followed by
another 4~byte number field that contains a transaction ID, which
starts at 1 and is incremented for each query sent, and all messages
sent in response to that query will contain the same transaction ID.
This is followed by a 2~byte number field containing the message type,
a 1~byte number field containing the number of argument fields present
in the message, and a blob field containing a series of bytes which
identify the types of each argument field. This blob is always twelve
bytes long, regardless of how few arguments there are (and presumably
this means no message ever has more than twelve arguments). Tag bytes
past the actual argument count of the message are set to 0.
...
\notebox{It seems that to reliably get data back when requesting
...
\tipbox{We also know how to request Nxs2-style higher resolution and
  color waveforms, see Section~\ref{sec:nxs2Waveforms} for details.}
...
{\tt (db/request-waveform-preview p2 3 1060)}

Replace the arguments with the {\tt var} holding your player
connection, the proper $S_r$ number for the media slot the track is
found in, and the $rekordbox$ ID of the track, and dysentery will open
a window like Figure~\ref{fig:waveformPreviewExample} showing the
waveform preview.
...
pre-Nexus CDJ 900 players as shown in
Figure~\ref{fig:tinyPreviewExample}. Our best guess is that for each
byte, the four low-order bits encode the height of the waveform in
that column, and the high-order bits may encode saturation again?

\begin{figure}
  \vspace{5mm}
  \centering
  \includegraphics[width=6.24cm]{assets/Tiny-preview-example}
  \caption{CDJ 900 waveform preview}
  \label{fig:tinyPreviewExample}
\end{figure}
...
\subsection{Requesting Nxs2 Track Waveforms}
\label {sec:nxs2Waveforms}

Thanks to some wonderful
analysis\footnote{\url{https://github.com/Deep-Symmetry/dysentery/issues/9}}
by @jan2000\footnote{\url{https://github.com/jan2000}} we now know how
to request and interpret the higher resolution and color waveforms
...
content from the {\tt ANLZ0000.EXT} file created by rekordbox. See the
Crate Digger
project\footnote{\url{https://github.com/Deep-Symmetry/crate-digger}}, which
has its own analysis document, if you would like to learn more about
the structure and content of these files.

To request the enhanced waveform preview of a track, send a message
with $type$ {\tt 2c04} containing the $rekordbox$ ID of the track, the
tag type identifier {\tt PWV4} and the file extension identifier {\tt
  EXT} encoded as four-byte numbers, holding the ASCII in big-endian
(backwards) order, as shown in
Figure~\ref{fig:nxs2WaveformPreviewRequest}.
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      ...
      \colorbitbox{lightgreen}{1}{\tt 11} & \colorbitbox{lightgreen}{4}{\tt 872349ae} &
      \colorbitbox{yellow}{1}{\tt 11} & \colorbitbox{yellow}{4}{$TxID$} &
      \colorbitbox{lightred}{1}{\tt 10} & \colorbitbox{lightred}{2}{\tt 2c04} &
      \colorbitbox{lightcyan}{1}{\tt 0f} & \colorbitbox{lightcyan}{1}{\tt 04} &
      \colorbitbox{lightpurple}{1}{\tt 14}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt 11} & \bitboxes*{1}{{$D$} {\tt 08} {$S_r$} {\tt 01}} &
      \bitbox{1}{\tt 11} & \bitbox{4}{$rekordbox$} &
      \bitbox{1}{\tt 11} & \bitbox{4}{\tt 34 56 57 50} & \bitbox{1}{\tt 11}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 30}
      \bitbox{4}{\tt 00 54 58 45} &
      \bitbox[]{12}{}
    \end{leftwordgroup}

  \end{bytefield}
  \caption{Nxs2 waveform preview request message}
  \label{fig:nxs2WaveformPreviewRequest}
\end{figure}
...
in which the track being asked about can be found, and has the same
values used in CDJ status packets, as shown in
Figure~\ref{fig:cdjStatus}. For some reason the value after $D$, which
identifies the location of the menu for which data is being loaded, is
1 in this case. $rekordbox$ identifies the specific track whose
analysis you are requesting, as found in a CDJ status packet or
playlist response, and the final two numeric arguments specify that
you are interested in the {\tt PWV4} tag (which holds the enhanced
waveform preview) from the track's {\tt EXT} extended analysis file.

The response will be a message with $type$ {\tt 4f02}, containing five
arguments. The first argument echoes back our request type, which was
{\tt 2c04}. The second always seems to be 0. The third contains the
length of the requested tag (holding the enhanced waveform preview) in
bytes. If this value is 0, the fourth argument will be omitted from
the response. When present, the fourth argument is a blob containing
the actual bytes of the enhanced waveform preview, as shown in
Figure~\ref{fig:nxs2WaveformPreviewResponse}. The fifth argument has
an unknown purpose and so far seems to always have the value 0.
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      ...
      \colorbitbox{lightgreen}{1}{\tt 11} & \colorbitbox{lightgreen}{4}{\tt 872349ae} &
      \colorbitbox{yellow}{1}{\tt 11} & \colorbitbox{yellow}{4}{$TxID$} &
      \colorbitbox{lightred}{1}{\tt 10} & \colorbitbox{lightred}{2}{\tt 4f02} &
      \colorbitbox{lightcyan}{1}{\tt 0f} & \colorbitbox{lightcyan}{1}{\tt 05} &
      \colorbitbox{lightpurple}{1}{\tt 14}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00002c04} & \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00000000} &
      \bitbox{1}{\tt 11} & \bitbox{4}{$length$} & \bitbox{1}{\tt 14}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 30}
      \bitbox{4}{$length$} & \bitbox[lrt]{12}{Extended waveform preview bytes}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{}
      ...
      \skippedwords \\
      \wordbox[lr]{1}{} \\
      \bitbox[lrb]{5}{} & \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00000000} & \bitbox[lt]{6}{}
    \end{leftwordgroup}

  \end{bytefield}
  \caption{Nxs2 waveform preview response message}
  \label{fig:nxs2WaveformPreviewResponse}
\end{figure}

The extended preview data begins at byte {\tt 34} and is 7,200
(decimal) bytes long, representing 1,200 columns of waveform preview
information.
...
to obtain the scrollable detail view is {\tt PWV5}. The full request
is shown in Figure~\ref{fig:nxs2WaveformDetailRequest}.
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt 11} & \bitboxes*{1}{{$D$} {\tt 08} {$S_r$} {\tt 01}} &
      \bitbox{1}{\tt 11} & \bitbox{4}{$rekordbox$} &
      \bitbox{1}{\tt 11} & \bitbox{4}{\tt 35 56 57 50} & \bitbox{1}{\tt 11}
    \end{leftwordgroup} \\
  ...
  \end{bytefield}
  \caption{Nxs2 waveform detail request message}
  \label{fig:nxs2WaveformDetailRequest}
\end{figure}
...
analysis you are requesting, as found in a CDJ status packet or
playlist response, and the final two numeric arguments specify that
you are interested in the {\tt PWV5} tag (which holds the enhanced
waveform detail) from the track's {\tt EXT} extended analysis file.

The response will be a message with $type$ {\tt 4f02}, containing five
arguments. The first argument echoes back our request type, which was
{\tt 2c04}. The second always seems to be 0. The third contains the
length of the requested tag (holding the enhanced waveform detail) in
bytes. If this value is 0, the fourth argument will be omitted from
the response. When present, the fourth argument is a blob containing
the actual bytes of the enhanced waveform segments, as shown in
Figure~\ref{fig:nxs2WaveformDetailResponse}. The fifth argument has an
unknown purpose and so far seems to always have the value 0.
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 30}
      \bitbox{4}{$length$} & \bitbox[lrt]{12}{Extended waveform detail bytes}
    \end{leftwordgroup} \\
  ...
  \end{bytefield}
  \caption{Nxs2 waveform detail response message}
  \label{fig:nxs2WaveformDetailResponse}
\end{figure}

The extended waveform detail data begins at byte {\tt 34} and has a
...
and there are 150 segments per second of audio. (These seem to
correspond to ``half frames'' counted as {\tt 03.5{\tiny F}} following
the seconds in the player display.) Each pair of bytes encodes the
height of the waveform at that segment as a five bit value, along with
three bits each of red, green, and blue intensity, arranged as shown
in Figure~\ref{fig:nxs2WaveformDetailSegment}.
...
\begin{figure}
  ...
  \end{bytefield}
  \caption{Nxs2 waveform detail segment bits}
  \label{fig:nxs2WaveformDetailSegment}
\end{figure}


\subsection{Requesting Cue Points and Loops}
\label{sec:nexusCuePoints}

\notebox{This section discusses how to obtain cue points and loops
  which are compatible with original nexus players. See
  Section~\ref{sec:nxs2CuePoints} for how you can obtain a newer
  format which includes hot cue colors, DJ-assigned comment text, and
  hot cues beyond C.}
...
only $cue$ has a meaning, and it identifies the position of the cue
point in the track, in \( \frac{1}{150} \) second units. For loops,
$cue$ identifies the start of the loop, and $loop$ identifies the end
of the loop, again in \( \frac{1}{150} \) second units.
...
\subsection{Requesting Nxs2 Cue Points and Loops}
\label{sec:nxs2CuePoints}
...
in Section~\ref{sec:nexusCuePoints}, you can learn about any custom
colors a DJ has assigned to their hot cues, as well as optional text
comments describing hot cues, mamory points, and loops. And while
older players only supported hot cues A through C, this new format
returns more.

\tipbox{In order to work well in mixed-player environments, the
  firmware of even older players has been updated to return this
  information if it is found in the exported data, so it is worth
  trying to ask for it. If the query described in this section fails,
  then you can fall back on the one shown in
  Section~\ref{sec:nexusCuePoints}.}
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      ...
      \colorbitbox{lightgreen}{1}{\tt 11} & \colorbitbox{lightgreen}{4}{\tt 872349ae} &
      \colorbitbox{yellow}{1}{\tt 11} & \colorbitbox{yellow}{4}{$TxID$} &
      \colorbitbox{lightred}{1}{\tt 10} & \colorbitbox{lightred}{2}{\tt 2b04} &
      \colorbitbox{lightcyan}{1}{\tt 0f} & \colorbitbox{lightcyan}{1}{\tt 03} &
      \colorbitbox{lightpurple}{1}{\tt 14}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt 11} & \bitboxes*{1}{{$D$} {\tt 08} {$S_r$} {\tt 01}} &
      \bitbox{1}{\tt 11} & \bitbox{4}{$rekordbox$} & \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00000000} & \bitbox[]{1}{}
    \end{leftwordgroup}

  \end{bytefield}
  \caption{Extended cue point request message}
  \label{fig:cuePointRequestPacket2}
\end{figure}

Enhanced information about the cue points and loops stored in a track
can be obtained with a request like the one shown in
Figure~\ref{fig:cuePointRequestPacket2}.\footnote{Thanks to Matt
  Positive, \url{https://soundcloud.com/mattpositive}, for packet
  captures!}
...
The response will be a message with $type$ {\tt 4e02}, containing
five arguments. The first argument echoes back our request type, which
was {\tt 2b04}. The second always seems to be 0. The third contains
the length of the blob containing cue and loop points in bytes, which
seems redundant, because that is also conveyed in the fourth argument
itself, which is a blob containing the actual bytes of the cue and
loop points, as shown in Figure~\ref{fig:cuePointResponse2}.
However, if there are no cue or loop points, this value will be 0, and
the following blob field will be completely omitted from the response,
so you \emph{must not} try to read it!
...
begins. The extended entry structure is shown in
Figure~\ref{fig:cuePointEntry2}.
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      ...
      \colorbitbox{lightgreen}{1}{\tt 11} & \colorbitbox{lightgreen}{4}{\tt 872349ae} &
      \colorbitbox{yellow}{1}{\tt 11} & \colorbitbox{yellow}{4}{$TxID$} &
      \colorbitbox{lightred}{1}{\tt 10} & \colorbitbox{lightred}{2}{\tt 4e02} &
      \colorbitbox{lightcyan}{1}{\tt 0f} & \colorbitbox{lightcyan}{1}{\tt 05} &
      \colorbitbox{lightpurple}{1}{\tt 14}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00002b04} & \bitbox{1}{\tt 11} & \bitbox{4}{\tt 00000000} &
      \bitbox{1}{\tt 11} & \bitbox{4}{$length_1$} & \bitbox{1}{\tt 14}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{}
      ...
      \skippedwords \\
      \wordbox[lr]{1}{} \\
      \bitbox[lrb]{5}{} & \bitbox{1}{\tt 11} & \bitbox{4}{$num_{entries}$} & \bitbox[lt]{6}{}
    \end{leftwordgroup}

  \end{bytefield}
  \caption{Extended cue point response message}
  \label{fig:cuePointResponse2}
\end{figure}
...
labeled $H$, is {\tt 00} for ordinary cue points, but has a value if
this entry defines a hot cue. Hot cues A through H are represented by
the values {\tt 01} through {\tt 08}.

$F_l$, at byte~{\tt 06} has the value {\tt 01} if this entry specifies
a memory point and {\tt 02} if it is a loop. If both $H$ and $F_l$ are
{\tt 00}, the entry should be ignored (it is probably a leftover cue
point that was deleted by the DJ).

The actual location of the cue and loop points are in the values $cue$
and $loop$. These are both 4-byte integers, and as noted above, they
are sent in a little-endian byte order. For non-looping cue points,
only $cue$ has a meaning, and it identifies the position of the cue
point in the track, in \( \frac{1}{150} \) second units. For loops,
$cue$ identifies the start of the loop, and $loop$ identifies the end
of the loop, again in \( \frac{1}{150} \) second units.

$len_c$ at byte~{\tt 48} is a two-byte, little-endian integer that
contains the length of the comment. If it is zero, there is no
comment. Otherwise, $comment$ will follow $len_c$, taking up that many
bytes, as a string (in UTF-16 encoding with a trailing {\tt 0000}
character). So if $len_c$ isn't zero, it will be an even number with
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      \bitbox{4}{$length$}
      \bitbox{1}{$H$} & \bitbox{1}{\tt 00} & \bitbox{1}{$F_l$} &
      \bitbox{5}{$unknown$} & \bitbox{4}{$cue$}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 10}
      \bitbox{4}{$loop$} & \bitbox[lrt]{12}{$unknown_2$}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 40}
      \bitbox[lrb]{8}{} & \bitbox{2}{$len_c$} & \bitbox[lrt]{6}{$comment$}
    \end{leftwordgroup} \\
  ...
  \end{bytefield}
  \caption{Extended cue/loop point entry}
  \label{fig:cuePointEntry2}
\end{figure}

Immediately after $comment$ (in other words, starting $len_c + {\tt
  4a}$ past the start of the entry) there are four one-byte values
containing color information. $c$ appears to be a code identifying the
...
next three bytes, $r$, $g$, and $b$, make up an RGB color
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 00}
      ...
      \colorbitbox{lightgreen}{1}{\tt 11} & \colorbitbox{lightgreen}{4}{\tt 872349ae} &
      \colorbitbox{yellow}{1}{\tt 11} & \colorbitbox{yellow}{4}{\tt fffffffe} &
      \colorbitbox{lightred}{1}{\tt 10} & \colorbitbox{lightred}{2}{\tt 0100} &
      \colorbitbox{lightcyan}{1}{\tt 0f} & \colorbitbox{lightcyan}{1}{\tt 00} &
      \colorbitbox{lightpurple}{1}{\tt 14}
    \end{leftwordgroup} \\
...
\begin{longtabu}{rX[4,l]X[5]}
  ...
  {\tt 1702} & Albums for Remixer &
  \rmst, sort, artist id \\

  \hline {\tt 1201} & Albums for Genre and Artist &
  ...
  {\tt 1802} & Tracks for Remixer and Album &
  \rmst, sort, artist id, album id\textsuperscript{*} \\

  & & {\tiny\textsuperscript{*}Use -1 for ``all''.} \\

  \hline {\tt 1301} & Tracks for Genre, Artist, and Album &
  \rmst, sort, genre id, artist id,\textsuperscript{*}
  ...
  \hline {\tt 1105} & Playlist Menu\footnote{See Section~\ref{sec:playlists}.} &
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
      \bitbox{4}{IP address} & \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$D_r$} &
      & \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$S_r$}
    \end{leftwordgroup} \\
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 20}
      \bitbox{1}{\tt \textbf{00}} & \bitbox{1}{$D$} & \bitbox{2}{$len_r$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$D_r$} &
      \bitboxes*{1}{{\tt 00} {\tt 00} {\tt 00}} & \bitbox{1}{$S_r$} &
      \bitbox[lrt]{4}{}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 30}
      \bitbox[lr]{16}{Media Name (UTF-16, padded with {\tt 0000})}
    \end{leftwordgroup} \\
    ...
    \begin{leftwordgroup}{\tiny\bfseries 60}
      \bitbox[lrb]{12}{} & \bitbox[lrt]{4}{}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 70}
      \bitbox[lr]{16}{Creation Date (UTF-16, padded with {\tt 0000})}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 80}
      \bitbox[lrb]{4}{} & \bitbox[lrt]{12}{}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries 90}
      \bitbox[lr]{16}{unknown UTF-16 text (sometimes ``1000'')}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries a0}
      \bitbox[lrb]{6}{} & \bitbox{2}{$tracks$} & \bitbox{1}{$x$} & \bitbox{1}{\tt 00} &
      \bitbox{1}{$T_r$} & \bitbox{1}{$a$} & \bitboxes*{1}{{\tt 00} {\tt 00}} &
      \bitbox{2}{$plists$}
    \end{leftwordgroup} \\

    \begin{leftwordgroup}{\tiny\bfseries b0}
      \bitbox[lrb]{8}{Total space} & \bitbox{8}{Free space}
    \end{leftwordgroup} \\
...
\label{sec:dealingWithFourPlayers}
