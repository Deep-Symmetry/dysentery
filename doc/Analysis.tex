...
adjustment, calculated from $Pitch$ as described above.\footnote{Since
  the mixer always reports a pitch adjustment of $+0\%$, its $BPM$
  value can be used directly without this additional step.}
...
\begin{figure}[h]
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt]{16}
    ...
    \begin{leftwordgroup}{\tiny 48} % 0x30
      ...
      \bitbox{1}{$X$} & \bitbox{1}{$B_b$} & \bitbox[]{7}{}
    \end{leftwordgroup} \\
  \end{bytefield}
...
``0.1~bars''. On the beat on which the cue point was saved the value
is {\tt 0x0000} and the CDJ displays ``0.0~Bars''. On the next beat,
...
\subsection{Extra CDJ Packets}

It was just discovered that registering a virtual CDJ also causes the
other CDJs to start sending unicast packets to that address on port
50000, in addition to the packets they normally broadcast on that
port. Dysentery is in the process of being updated to display these
packets as well, so they can be analyzed to see what other information
can be used.
