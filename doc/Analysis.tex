...
\begin{longtabu}{rlX}
  ...
  \rmst, sort, playlist or folder ID, type~(0:playlist; 1:folder) \\
  ...
  {\tt 1201} & Albums for Genre+Artist &
  \rmst, sort, genre ID, artist ID\footnote{Use -1 for ``all.''\label{footnote:minus1forAll}} \\

  {\tt 1206} & Tracks for BPM+Distance &
  \rmst, sort, bpm ID, distance (+/- \%; can be 0--6) \\

  {\tt 1214} & Tracks for Key+Distance &
  \rmst, sort, key ID, distance \\

  {\tt 1300} & Search by substring &
  \rmst, sort, search string byte size, search string (uppercase), unknown (0) \\

  {\tt 1301} & Tracks for (see args) &
  \rmst, sort, genre ID, artist ID\textsuperscript{\ref{footnote:minus1forAll}},
  album ID\textsuperscript{\ref{footnote:minus1forAll}} \\
