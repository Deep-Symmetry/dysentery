...
time with the flashing USB indicator LED on the player. Byte~107,
$S_a$, is the same kind of activity indicator for the SD slot.
Byte~111 ($U_l$ for ``USB local'') has the value 4 when there is no
USB media loaded, 0 when USB is loaded, and 2 when the USB Stop button
has been pressed and the USB media is being unmounted.

Byte~115 ($S_l$ for ``SD local'') has the value 4 when there is no SD
media loaded, 0 when SD is loaded, and 2 when the SD door has been
opened and the SD media is being unmounted.
...
\item[2] A track is in the process of loading.
...
\item[7] Cue Play is in progress (playback while the cue button is
  held down).
\item[8] Cue scratch is in progress.
...
\begin{figure}
  ...
  \end{bytefield}
  \caption{Track metadata menu header response}
...
\begin{description}
\item[1] Numeric, a parent ID, such as an artist for a track
  item.
\item[2] Numeric, the main ID, such as $rekordbox$ for a
  track item.
\item[3] Numeric, the length in bytes of Label 1.
\item[4] String, Label 1 (main text, such as the track title or
  artist name, as appropriate for the item type).
\item[5] Numeric, the length in bytes of Label 2.
\item[6] String, Label 2 (secondary text, e.g. for playlist entries
  the artist name, and Label 1 holds the title).
\item[7] Numeric, the type of this item (see
  Section~\ref{sec:menuItemTypes}).
\item[8] Numeric, some sort of flags field, details still unclear.
\item[9] Numeric, holds the album art ID when the type is a Track
  Title.
\item[10] Numeric, playlist position when relevant, e.g. when listing
  a playlist.
\item[11] Numeric, purpose unknown.
\item[12] Numeric, purpose unknown.
\end{description}
...
\begin{figure}
  ...
  \end{bytefield}
  \caption{Track metadata item response}
...
the track title text, and argument 8 holds the album $artwork$ ID if
...
\begin{figure}
  ...
  \end{bytefield}
  \caption{Track metadata menu footer response}
...
\begin{description}
\item[\tt 0x0001] Folder (such as in the playlists menu, a nested list
  of playlists rather than an individual playlist)
\item[\tt 0x0002] Album title
\item[\tt 0x0003] Disc
\item[\tt 0x0004] Track Title
\item[\tt 0x0006] Genre
\item[\tt 0x0007] Artist
\item[\tt 0x0008] Playlist
\item[\tt 0x000a] Rating
\item[\tt 0x000b] Duration (in seconds)
\item[\tt 0x000d] Tempo
\item[\tt 0x000f] Key
\item[\tt 0x0013] Color
\item[\tt 0x0023] Comment
\item[\tt 0x002e] Date Added
\item[\tt 0x0704] Track List Entry
\end{description}
...
Figure~\ref{fig:artworkResponse}.
...
Figure~\ref{fig:beatGridResponse}.
...
is encoded as a 4-byte big-endian integer at bytes 21--24. Subsequent
beats are found at 16-byte intervals, so the second $B_b$ value is
found at byte 36, and the second beat's time, in milliseconds from the
start of the track, is the big-endian integer at bytes 37--40. The
$B_b$ value for the third beat is at byte 52, and its millisecond time
is at bytes 53--56, and so on.
...
\subsection{Requesting the Waveform Overview}
...
this query is a good starting point.
...
caching, you can navigate the playlist folder hierarcy and deal with
only speific playlists.
...
Received 2  > Transaction: 2, message type: 0x4101
 (rendered menu item), argument count: 12, arguments:
  ...
  string: "Escape ft. Zoe Phillips" [label 1]
