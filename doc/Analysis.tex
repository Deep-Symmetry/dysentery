...
300 pixels long, and the detailed waveform, which uses 150 pixels per
second of track content. Although Austin has figured out how to
interpret the bytes for non-color waveforms, all of my players seem to
return an expanded format that we have not yet figured out how to
interpret and draw properly, so this section is incomplete.
...
the length of the waveform preview in bytes, which seems redundant,
because that is also conveyed in the fourth argument itself, which is
a blob containing the actual bytes of the waveform preview, as shown
in Figure~\ref{fig:waveformPreviewResponse}.
...
color information for players that support colored waveforms?
...
\subsection{Requesting Cue Points}

The locations of the cue points stored in a track can be obtained with
another request. The details will be documented once it is implemented
in dysentery.
